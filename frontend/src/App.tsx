  ...
  return (
    <div className="min-h-dvh flex overflow-hidden antialiased relative noise-bg ambient-bg">
      ...
      <div className="flex-1 md:ml-[260px] flex flex-col h-dvh relative">
        ...
        {view === 'content' && <ContentView navTo={navTo} currentJob={currentJob} refreshJob={() => currentJob && refreshCurrentJob(currentJob.id)} />}
        {view === 'media' && <MediaView navTo={navTo} currentJob={currentJob} />}
      </div>
    </div>
  );
}
...
function Sidebar({ view, navTo, jobs, currentJob, loadJob }: { view: ViewState, navTo: (v: ViewState) => void, jobs: Job[], currentJob: Job | null, loadJob: (id: string) => void }) {
  const navItems: { key: ViewState; icon: React.ReactNode; label: string }[] = [
    ...
    { key: 'media', icon: <Factory className="w-[18px] h-[18px]" />, label: 'Factory' },
  ];

  return (
    <aside className="w-[260px] h-full fixed left-0 top-0 bg-base-900/80 backdrop-blur-2xl hidden md:flex flex-col z-50 overflow-y-auto border-r border-white/[0.04]">
      <div className="flex items-center gap-3 px-6 pt-7 pb-6">
        <div className="w-9 h-9 rounded-xl bg-gradient-to-br from-accent-500 to-accent-600 flex items-center justify-center shadow-[0_2px_12px_rgba(245,158,11,0.3)]">
      ...
      <nav className="flex flex-col gap-1 px-4 flex-grow">
        ...
        {navItems.map(item => (
          <button key={item.key} onClick={() => navTo(item.key)}
            className={`flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 group relative ${view === item.key ? 'bg-accent-500/10 text-accent-400' : 'text-base-400 hover:text-base-200 hover:bg-white/[0.03]'}`}>
        ...
        <button className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-base-500 hover:text-base-300 hover:bg-white/[0.03] transition-all">
          <History className="w-[18px] h-[18px]" /> <span className="text-sm font-medium">History</span>
        </button>
        <button className="flex items-center gap-3 px-3 py-2.5 rounded-xl text-base-500 hover:text-base-300 hover:bg-white/[0.03] transition-all mt-auto">
      ...
      <div className="px-4 pb-6 pt-4">
        <div className="flex justify-between items-center mb-3 px-2">
          ...
          <button onClick={() => window.location.reload()} className="text-base-500 hover:text-accent-400 transition-colors p-1 rounded-lg hover:bg-white/[0.04]" title="Refresh"><RefreshCw className="w-3 h-3" /></button>
        </div>
        <div className="flex flex-col gap-1.5 max-h-[30vh] overflow-y-auto stagger">
          {jobs.slice(0, 10).map((job) => (
            <button key={job.id} onClick={() => loadJob(job.id)}
              className={`w-full text-left rounded-xl px-3 py-2.5 text-sm flex justify-between items-center transition-all duration-200 ${currentJob?.id === job.id ? 'bg-accent-500/8 border border-accent-500/20 text-base-100' : 'text-base-400 hover:text-base-200 hover:bg-white/[0.03] border border-transparent'}`}>
...
function TopNav({ view }: { view: ViewState }) {
  return (
    <nav className="sticky top-0 w-full z-40 bg-base-950/60 backdrop-blur-xl flex justify-between items-center px-6 md:px-8 py-3 border-b border-white/[0.04] h-14 shrink-0">
      <div className="flex items-center gap-4">
        ...
        {view === 'content' && (
          <div className="hidden md:flex gap-1 text-sm">
            ...
            <button className="px-3 py-1.5 rounded-lg text-base-400 hover:text-base-200 hover:bg-white/[0.03] transition-all text-[13px]">Library</button>
          </div>
        )}
      </div>
      <div className="flex items-center gap-2">
        ...
        <button className="p-2 rounded-xl text-base-400 hover:text-accent-400 hover:bg-white/[0.03] transition-all hidden md:flex"><Settings className="w-[18px] h-[18px]" /></button>
        <button className="md:hidden p-2 rounded-xl text-base-400"><Menu className="w-[18px] h-[18px]" /></button>
        <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-accent-600 to-accent-700 flex items-center justify-center ml-1 cursor-pointer text-[11px] font-bold text-base-950 shadow-[0_2px_8px_rgba(245,158,11,0.2)]">SE</div>
      </div>
    </nav>
  );
}
      ...
      <div className="flex-1 overflow-y-auto pr-2 md:pr-4 flex flex-col gap-5 pb-40 scroll-smooth stagger">
        {currentJob && (
          <div className="self-end max-w-2xl w-full">
            <div className="bg-gradient-to-br from-accent-700 to-accent-600 p-5 rounded-2xl rounded-tr-sm text-base-950 shadow-lg shadow-accent-500/10">
        ...
        {events.map((event, i) => (
          <div key={i} className="self-start max-w-3xl flex gap-3 w-full">
            <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${event.status === 'error' ? 'bg-signal-error-dim' : 'bg-base-800 border border-white/[0.06]'}`}>
            ...
            <div className="glass-panel p-4 rounded-2xl rounded-tl-sm flex-1">
              ...
              <p className="text-base-200 text-sm flex items-center gap-2 leading-relaxed">
                {event.status === 'working' || event.status === 'started' ? <RefreshCw className="w-3.5 h-3.5 text-accent-400 animate-spin flex-shrink-0" />
                 : event.status === 'error' ? <span className="text-signal-error font-bold text-xs">ERR</span>
                 : <Bot className="w-4 h-4 text-signal-success flex-shrink-0" />}
                {event.message}
              </p>
            </div>
          </div>
        ...
        {isAwaitingApproval && currentJob?.plan && (
          <div className="self-start max-w-3xl flex gap-3 w-full mt-2">
            <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 bg-signal-warning-dim">
            ...
            <div className="glass-panel p-6 rounded-2xl rounded-tl-sm flex-1 border border-signal-warning/20">
              ...
              <div className="bg-base-900 p-4 rounded-xl border border-white/[0.04] mb-4">
              ...
              <div className="flex flex-col gap-3">
                <textarea className="w-full bg-base-800 border border-white/[0.06] rounded-xl p-3 text-sm text-base-100 focus:outline-none focus:border-accent-500/40 resize-none h-20 placeholder:text-base-500"
                ...
                <div className="flex gap-2 justify-end">
                  {feedback.trim().length > 0 && (
                    <button onClick={() => { handleRevisePlan(currentJob.id, feedback); setFeedback(''); }}
                      className="px-4 py-2 bg-base-700 hover:bg-base-600 text-base-200 rounded-xl text-sm font-semibold transition-colors border border-white/[0.06]">Revise Plan</button>
                  )}
      ...
      {(!currentJob || currentJob.status === 'completed' || currentJob.status === 'failed') && (
        <div className="absolute bottom-6 left-6 md:left-8 right-6 md:right-8 z-30">
          <div className="glass-panel rounded-2xl p-4 flex flex-col gap-3 max-w-4xl mx-auto bg-base-900/90">
            <div className="flex gap-2 px-1 overflow-x-auto pb-1">
              {['professional', 'conversational', 'technical', 'educational'].map(t => (
                <button key={t} onClick={() => setTone(t)}
                  className={`px-3.5 py-1.5 rounded-lg text-xs whitespace-nowrap capitalize font-medium transition-all ${tone === t ? 'text-accent-400 bg-accent-500/10 border border-accent-500/20' : 'text-base-400 hover:text-base-200 border border-transparent hover:bg-white/[0.03]'}`}>{t}</button>
              ))}
              <div className="ml-auto items-center gap-2 text-xs text-base-400 bg-base-800 px-3 py-1.5 rounded-lg whitespace-nowrap hidden sm:flex border border-white/[0.04]">
            ...
            <div className="flex items-end gap-2 bg-base-800 rounded-xl p-2 border border-white/[0.06] focus-within:border-accent-500/30 transition-colors">
              <button className="p-2.5 text-base-500 hover:text-accent-400 transition-colors hidden sm:block rounded-lg hover:bg-white/[0.03]">
              ...
              <button onClick={submitJob} disabled={!topicInput.trim()}
                className={`btn-primary w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 group ${!topicInput.trim() ? 'opacity-40 cursor-not-allowed !shadow-none !transform-none' : ''}`}>
